...
type cb_getTask = (error: grpc.ServiceError | null, response: tasks.GetTaskResponse) => void;
type cb_updateTask = (error: grpc.ServiceError | null, response: tasks.UpdateTaskResponse) => void;
type cb_poll = (error: grpc.ServiceError | null, response: tasks.PollResponse) => void;
type cb_getTaskLogs = (error: grpc.ServiceError | null, response: tasks.GetTaskLogsResponse) => void;
...
jest.mock("../proto/service/task_service_grpc_pb", () => {
    ...
    return {
        ...
        return {
            ...
            getTask: (req:tasks.GetTaskRequest, cb:cb_getTask) => { cb(null, new tasks.GetTaskResponse()) },
            updateTask: (req:tasks.UpdateTaskResponse, cb:cb_updateTask) => { cb(null, new tasks.UpdateTaskResponse()) },
            poll: (req:tasks.PollRequest, cb:cb_poll) => { cb(null, new tasks.PollResponse()) },
            getTaskLogs: (req:tasks.GetTaskLogsResponse, cb:cb_getTaskLogs) => { cb(null, new tasks.GetTaskLogsResponse()) }
        };
      }),
    };
  });
